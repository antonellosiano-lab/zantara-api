...
export async function triggerScenarioHandler(req, res) {
  const route = "/api/make/trigger-scenario";
  ...
  if (!scenario_id || !webhook_url || !payload) {
    log({ route, action: "validation", status: 400, message: "Missing scenario_id, webhook_url, or payload", userIP });
  ...
  try {
    ...
    log({ route, action: "success", status: 200, message: "Scenario triggered", userIP });
